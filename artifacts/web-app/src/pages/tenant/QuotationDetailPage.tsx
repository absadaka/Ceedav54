      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-border bg-background p-4 space-y-2.5">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Quote details</p>
            <dl className="space-y-1.5 text-sm">
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Advisor</dt>
                <dd className="font-medium">{qt.advisor_name ?? <span className="text-muted-foreground italic">—</span>}</dd>
              </div>
              {qt.estimated_hours && (
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Est. hours</dt>
                  <dd className="font-medium flex items-center gap-1"><Clock className="w-3 h-3" />{qt.estimated_hours} h</dd>
                </div>
              )}
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Created</dt>
                <dd>{fmt(qt.created_at)}</dd>
              </div>
              {qt.expires_at && (
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Valid until</dt>
                  <dd>{fmt(qt.expires_at)}</dd>
                </div>
              )}
              {qt.sent_at && (
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Sent</dt>
                  <dd>{fmt(qt.sent_at)}</dd>
                </div>
              )}
              {qt.approved_at && (
                <div className="flex justify-between">
                  <dt className="text-muted-foreground text-green-700">Approved</dt>
                  <dd className="text-green-700 font-medium">{fmt(qt.approved_at)}</dd>
                </div>
              )}
              {qt.rejected_at && (
                <div className="flex justify-between">
                  <dt className="text-muted-foreground text-red-600">Rejected</dt>
                  <dd className="text-red-600">{fmt(qt.rejected_at)}</dd>
                </div>
              )}
            </dl>
          </div>
