...
const STATUS_META: Record<string, { label: string; color: string }> = {
  ...
  in_progress:{ label: "In Progress",color: "bg-violet-100 text-violet-800 border-violet-200" },
...
function dateRange(preset: string): { date_from?: string; date_to?: string } {
  const now = new Date();
  const d0  = (d: Date) => d.toISOString().slice(0, 10);
  if (preset === "today") {
...
export default function BookingsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["bookings", search, statusFilter, datePreset],
