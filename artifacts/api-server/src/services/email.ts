...
<div style="background:#fff;border-radius:16px;padding:48px;max-width:440px;text-align:center;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
  ...
  <p style="margin:0;font-size:13px;color:#a1a1aa;">${shopName} via ceer></p>
</div>
</body></html>`;
}
...
function buildFrom(shopName: string, config: TenantEmailConfig): string {
  const name = config.fromName || shopName || "ceer>";
...
export async function sendPlatformEmail({ to, subject, html }: { to: string; subject: string; html: string }) {
  ...
  const from = `ceer Platform <${DEFAULT_FROM_EMAIL}>`;
...
<tr><td style="background:#0a0a0a;padding:20px 32px;">
  <span style="color:#ffffff;font-size:22px;font-weight:700;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">ceer></span>
...
<tr><td style="padding:40px 32px;">
  ...
  <p style="margin:0 0 16px;font-size:15px;color:#3f3f46;line-height:1.6;">
    You've been added to the <strong>ceer></strong> platform admin console with the role of <strong>${roleLabel}</strong>.
  </p>
...
export function teamInviteEmailHtml(opts: {
  ...
  const credentialsBlock = opts.tempPassword
    ? `
      <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
        <tr><td style="padding:16px;background:#f4f4f5;border-radius:8px;">
          <p style="margin:0 0 8px;font-size:12px;color:#71717a;text-transform:uppercase;letter-spacing:0.5px;font-weight:600;">Your sign-in details</p>
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="font-size:13px;color:#71717a;padding:4px 0;">Email</td>
              <td align="right" style="font-size:13px;font-weight:600;color:#0a0a0a;padding:4px 0;">${opts.email}</td>
            </tr>
            <tr>
              <td style="font-size:13px;color:#71717a;padding:4px 0;">Temporary password</td>
              <td align="right" style="font-size:13px;font-weight:700;color:#0a0a0a;padding:4px 0;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;">${opts.tempPassword}</td>
            </tr>
            <tr>
              <td style="font-size:13px;color:#71717a;padding:4px 0;">Role</td>
              <td align="right" style="font-size:13px;font-weight:600;color:#0a0a0a;padding:4px 0;">${roleLabel}</td>
...
<tr><td style="background:#161aff;padding:20px 32px;">
  ...
  <span style="color:#c5c6ff;font-size:13px;margin-left:8px;">via ceer></span>
</td></tr>
<tr><td style="padding:40px 32px;">
  ...
  <p style="margin:0 0 16px;font-size:15px;color:#3f3f46;line-height:1.6;">
    You've been added to <strong>${opts.shopName}</strong> on ceer> as a <strong>${roleLabel}</strong>.
  </p>
  ${credentialsBlock}
  <table cellpadding="0" cellspacing="0" style="margin:0 0 32px;">
    <tr><td style="background:#161aff;border-radius:8px;">
      <a href="${opts.loginUrl}" style="display:inline-block;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 36px;">
        Sign in &rarr;
      </a>
    </td></tr>
...
<tr><td style="padding:20px 32px;background:#fafafa;border-top:1px solid #e4e4e7;">
  <p style="margin:0;font-size:12px;color:#a1a1aa;text-align:center;">
    ${opts.shopName} &middot; Powered by ceer>
  </p>
</td></tr>
</table>
</td></tr>
